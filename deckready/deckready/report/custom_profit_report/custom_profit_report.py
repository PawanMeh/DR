...
def execute(filters=None):
	...
	so_project_exp_map = get_so_project_map(filters)
	...
	for inv in invoice_list:
		...
		exp_amount = 0
		for so_exp in so_project_exp_map:
			if so_exp["Sales Order"] == row[5]:
				exp_amount = so_exp["Expenses"]
				
		row.append(exp_amount)
		gross_profit = row[9] - exp_amount
...
def get_columns(invoice_list):
	...
	columns = columns + income_columns + [_("Net Total") + ":Currency/currency:120"] + tax_columns + \
		...
		_("Expenses") + ":Currency/currency:120",_("Profit") + ":Currency/currency:120"]
...
def get_invoices(filters):
	...
	return frappe.db.sql("""select name, posting_date, customer, customer_name, remarks, 
...
def get_so_project_map(filters):
	so_project_details = frappe.db.sql("""select substring(name,-8) as "Sales Order", name as "Project" from `tabProject` 
							where substring(name,-8) in (select name from `tabSales Order`) 
							union 
							select substring(name,-10) as "Sales Order", name as "Project" from `tabProject` 
							where substring(name,-10) in (select name from `tabSales Order`)""",as_dict=1)
	so_project_map = frappe._dict()
	for d in so_project_details:
		so_project_map.setdefault(d.get("Sales Order"), d.get("Project"))
	data = []

	for sales_order, project in so_project_map.items():
		row = {
			"Sales Order": sales_order,
			"Project": project
		}
		row["Expenses"]= get_project_expenses(project,filters)
		data.append(row)
	return data
	
def get_project_expenses(project,filters):
	conditions=""
	...
	if project:
		...
		project_expenses = frappe.db.sql("""select b.project, sum(a.total_debit) from `tabJournal Entry` a, 
										`tabJournal Entry Account` b where a.name = b.parent and %s """% (conditions,),filters)
	
	return flt(project_expenses[0][1]) if project_expenses else 0
